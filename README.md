...
Proyecto: `16asym1c`. Dataset previsto: `production` (confirmar que existe y es público). Configura las variables del ejemplo; nunca subas tokens al repositorio. Para el Studio, configura también `SANITY_STUDIO_PROJECT_ID` y `SANITY_STUDIO_DATASET` en `.env` o en el entorno del terminal. Los valores por defecto ya apuntan al proyecto indicado.
...
Web compilada y comprobada en escritorio y móvil. Studio compilado y publicado; esquemas e imágenes importados. Pendiente: token de servidor para activar recepción de solicitudes, otros perfiles sociales, imágenes y URLs definitivas de proyectos, y credenciales Vercel/GitHub para el despliegue web. La sección de productos está excluida del código y del modelo editorial.
