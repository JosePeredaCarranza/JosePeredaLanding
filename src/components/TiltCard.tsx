...
export function TiltCard({children,index}:{children:ReactNode;index:number}) {
  ...
  function reset(){
    ...
    card.current?.style.setProperty('--light-x','-15%');
    card.current?.style.setProperty('--light-y','-15%');
  }
  function move(event:PointerEvent<HTMLDivElement>){
    ...
    card.current?.style.setProperty('--light-x',`${-15-x*12}%`);
    card.current?.style.setProperty('--light-y',`${-15-y*12}%`);
  }
